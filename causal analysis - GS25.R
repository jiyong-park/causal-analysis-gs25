...
quarterly_performance <- read.csv("quarterly performance.csv")

performance_GS25 <- read.csv("quarterly performance.csv") %>% 
  filter(company=="GS25")
...
TS_data <- zoo(cbind(performance_GS25$sale, performance_GS25$year, performance_GS25$quarter, performance_GS25$profit.margin), 1:14)
...
sales_trend1 <- quarterly_performance %>% filter(company=="GS25" | company=="CU") %>% 
...
sales_trend2 <- quarterly_performance %>% filter(company=="GS25" | company=="CU") %>%
  group_by(company) %>%
...
profit_trend1 <- quarterly_performance %>% filter(company=="GS25" | company=="CU") %>% 
...
profit_trend2 <- quarterly_performance %>% filter(company=="GS25" | company=="CU") %>%
  group_by(company) %>%
...
margin_trend1 <- quarterly_performance %>% filter(company=="GS25" | company=="CU") %>% 
...
margin_trend2 <- quarterly_performance %>% filter(company=="GS25" | company=="CU") %>%
  group_by(company) %>%
...
synthetic_output_profits %>% grab_synthetic_control()
